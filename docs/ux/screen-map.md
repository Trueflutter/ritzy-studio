...
| `/projects/[p]/rooms/[r]/brief/details` | form-step (1e·2) | both | Answer what I know; accelerators optional | Editorial question list | Continue to concepts | Back; floor-plan upload; measurement fields | saved-values re-render, rejection NEVER wipes input, measurements optional with the assumption list visible (12.5) beside them, refused-answer state naming the field, plan-uploaded state, room-identified state (from whole plan), plan-being-read state, plan-refused states (PDF, and below the readable resolution) |
...
- Floor-plan room identification is a state of `/brief/details`, composed from existing primitives: plan image + detected-room list as chips, the picked room named back in the panel. NO overlay: S5b measured the model's bounding boxes against a real Emaar plan and they were plausible and wrong (the living room's box enclosed the balcony), so nothing is drawn on the drawing and the plan opens full size instead. Interactive region drawing on the plan is NOT in scope; if ever wanted it needs a Claude Design pass (flagged, none needed now).
