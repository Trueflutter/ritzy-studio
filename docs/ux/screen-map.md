...
| `/projects/[p]/rooms/[r]/spec` PLANNED | detail + ledger (1c pattern) | both | Confirm what the design contains before sourcing | Object ledger (role, qty, size) beside the render | Confirm and source | Edit row; remove object; add note | extraction-running, extraction-failed, edited-dirty |
...
| `/projects/[p]/rooms/[r]/presentation` | detail (1d, on ink) | both + client-over-shoulder | See the finished room and buy it | Final hero render | Unlock shopping list (unpaid) / Open shopping list (paid) | View alternate/requested views; change pieces | render-queued/polling, failed+retry, paid vs unpaid |
