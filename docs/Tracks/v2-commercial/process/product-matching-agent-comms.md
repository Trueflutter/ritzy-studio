...
Sam/Chief: route the next exact PM-001 local/dev boundary if Product Matching should continue. Do not run another validation pass, make visual-sourcing runtime calls through the app flow, use controlled preview, invoke app actions, write outside approved implementation tests, broaden/runtime allowlists, perform production activity, make broad scoring rewrites, change prompt/runtime image-generation behavior, change curtains/textiles/thin pools/sofa/coffee/rug behavior, do unrelated quality work, touch floor-plan work, or add Catalog-First coupling without a fresh explicit route.
