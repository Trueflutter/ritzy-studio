...
Allowed scope: Post-PR #294 selected-vs-alternate consistency boundary only: local/dev Product Matching option-composition code/tests/docs so visible alternates use the same role-scoped candidate pools, role contracts, room/class/size filters, and design-fit envelope as selected/default products. Authorized work is limited to `packages/domain/src/product-matching.ts`, focused domain tests, `apps/web/app/actions.ts` alternate/refresh/substitution composition paths, and PM-001 evidence/coordination docs.
...
Expected next artifact: One focused PR for strict PR Review Agent review, with an `ARCHITECT_NOTE:` confirming scope, verification, and stop rules.
SLA: Review-requested; do not start additional PM-001 implementation unless strict review or Sam/Chief routes a fresh exact boundary.
Last architect instruction: PR #294 merged and improved selected/default product quality. Sam's latest QA found the selected item now aligns with the generated design, but alternate options in the same role can still look stale/generic because visible alternates used the flat global `visualRanked` stream instead of the role-scoped scorer. This boundary fixes selected-vs-alternate consistency only: same role, same design direction, different product family. It does not approve live validation, controlled preview, live app/catalog actions, catalogue/product mutation, DB/schema/generated type changes, deploy/flag changes, prompt/runtime image-generation behavior changes, payment/checkout, final render, floor-plan work, UI redesign, broad rewrite, or Catalog-First coupling.
Agent ack: Product Matching Agent acknowledged the selected-vs-alternate consistency route on branch `codex/pm001-alternate-consistency`; evidence lives in `docs/Tracks/v2-commercial/product-matching-evals/2026-06-01-pm001-selected-alternate-consistency-evidence.md`.
Current PR: pending focused implementation PR on `codex/pm001-alternate-consistency`; #295 remains the earlier docs-only post-#294 manual-QA route; #294 merged (<https://github.com/Trueflutter/ritzy-studio/pull/294>); #286 merged (<https://github.com/Trueflutter/ritzy-studio/pull/286>); #283 merged (<https://github.com/Trueflutter/ritzy-studio/pull/283>); #282 merged (<https://github.com/Trueflutter/ritzy-studio/pull/282>); #280 merged (<https://github.com/Trueflutter/ritzy-studio/pull/280>); #278 merged (<https://github.com/Trueflutter/ritzy-studio/pull/278>); #277 merged (<https://github.com/Trueflutter/ritzy-studio/pull/277>)
Blocker: None for review; waiting for strict PR Review Agent verdict after the focused PR opens.
