...
| Home office | Not visually tested | No existing read-only home-office/study/workspace concept was available; only a pool-only probe was recorded. |
...
- Home office did not receive visual arbitration coverage, despite being a PRD-required room type.
...
| Home office representative QA completed | Blocked | No real selected home-office concept was available without writes. |
...
2. Add or approve a representative home-office QA scenario, then rerun visual arbitration against a real concept image.
3. Continue catalog evidence and measurement enrichment so required anchor warnings become less frequent.
4. Rerun the manual QA report after targeted fixes and require all four room types to pass the QA gate before controlled default-off preview testing.
...
This decision does not pause the Product Matching Engine behind the catalog-first track. The evidence points to targeted Product Matching fixes and a missing home-office QA scenario, not a need to wait for catalog-first generation.
