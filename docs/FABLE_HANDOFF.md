...
Concept-first is locked (`docs/Tracks/mvp/adr/0001-concept-first-product-grounded-workflow.md`, Accepted): design a concept freely, then ground it in real products, then re-render against the chosen products. Fidelity ("what you see is buyable") is guaranteed by truth separation (`adr/0003`): the shopping list is database truth, never inferred from render pixels. A catalog-first path (build the concept FROM stock) once existed as an unwired scaffold in `packages/domain/src/catalog-first-*.ts`; slice S1 of the product pass retired it (Gate 1 approved disposition, 2026-09-01). The only surviving pieces are the room-type normalizer, now `apps/web/lib/room-type-normalize.ts`, and the spatial rules' narrowed role contract in `packages/domain/src/spatial-design-rules.ts`.
